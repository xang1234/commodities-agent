...
export const DAILY_CALL_STATUSES = ["draft", "approved", "published"] as const;
...
export type DailyCallBrief = {
  ...
  approved_at?: string;
  // The sealed snapshot is created at publish time, so it is absent on draft and
  // approved briefs and present only once published.
  snapshot_id?: string;
  published_at?: string;
};

export type ApproveDailyCallInput = {
  reviewer_user_id: string;
  approved_at: string;
};
...
export function approveDailyCall(draft: DailyCallBrief, input: ApproveDailyCallInput): DailyCallBrief {
  if (draft.status !== "draft") throw new Error("daily_call must be in draft status before approval");
  assertUuid(input.reviewer_user_id, "daily_call.reviewer_user_id");
  assertIsoDateTime(input.approved_at, "daily_call.approved_at");
...
export function publishDailyCall(approved: DailyCallBrief, input: PublishDailyCallInput): DailyCallBrief {
  if (approved.status !== "approved") throw new Error("daily_call must be approved before publishing");
